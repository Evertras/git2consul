...
var write_content_to_consul = function(client, key_name, content, cb) {
  ...
  client.kv.set({'key': key_name, value: content !== null ? String(content) : null, token: token}, function(err) {
    if (err) {
...
var delete_from_consul = function(client, key_name, cb) {
  ...
  client.kv.del({'key': key_name, token: token}, function(err) {
    if (err) {
...
Consul.prototype.file_deleted = function(branch, file, cb) {
  ...
  this.client.kv.del({'key': key_name, token: token, recurse: branch.expand_keys}, function(err) {
    /* istanbul ignore if */
...
Consul.prototype.get_kvs = function(branch, file, cb) {
  ...
  this.client.kv.get({'key': key_name + '/', token: token, recurse: true}, function(err, kvs, res) {
...
Consul.prototype.getLastProcessedRef = function(branch, cb) {
  ...
  this.client.kv.get({'key': key_name, token: token}, function(err, item) {
    /* istanbul ignore if */
    if (err) return cb(err);
