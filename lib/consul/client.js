...
function Consul(host, port, token, secure) {
  this.client = new ConsulClient({'host': host, 'port': port, 'secure': secure, defaults: { token: token }});
}

module.exports = Consul;

var write_content_to_consul = function(client, key_name, content, cb) {
  logger.trace('Adding key %s, value:\n%s', key_name, content);
  client.kv.set({'key': key_name, value: content !== null ? String(content) : null}, function(err) {
    if (err) {
...
var delete_from_consul = function(client, key_name, cb) {
  logger.trace('Deleting key %s', key_name);
  client.kv.del({'key': key_name}, function(err) {
    if (err) {
 ...
 * the diff.
 */
var populate_kvs_from_object = function(client, branch, prefix, obj, existing_kvs, cb) {
  var write_kvs = [];
  var delete_kvs = [];
  var candidate_kvs = [];
  ...
  var do_writes = _.after(delete_kvs.length, function(err) {
    ...
    if (write_kvs.length > 0) {
      ...
      write_kvs.forEach(function(write) {
        write_content_to_consul(client, write.key, write.value, cb);
      });
    } else {
      cb();
    }
  ...
  if (delete_kvs.length > 0) {
    delete_kvs.forEach(function(del) {
      delete_from_consul(client, del.key, do_writes);
    });
  } else {
    do_writes();
  }
 ...
 */
Consul.prototype.file_modified = function(branch, file, cb) {
  var self = this;
  ...
  var handle_json_kv_file = function(file_path, kvs, cb) {
      fs.readFile(file_path, {encoding: 'utf8'}, function (err, body) {
        ...
        try {
          ...
          populate_kvs_from_object(self.client, branch, create_key_name(branch, file), obj, kvs, cb);
        } catch (e) {
          logger.warn("Failed to parse .json file.  Using body string as a KV.");
          write_content_to_consul(self.client, create_key_name(branch, file), body, cb);
        }
      });
  };
  ...
  if (branch.expand_keys && branch.expand_keys_diff) {
    ...
    self.get_kvs(branch, file, function(err, kvs) {
    ...
    self.file_deleted(branch, file, function (err) {
 ...
 */
Consul.prototype.file_deleted = function(branch, file, cb) {
  ...
  this.client.kv.del({'key': key_name}, function(err) {
    /* istanbul ignore if */
 ...
 * [{'key': k, 'value': v}]
 */
Consul.prototype.get_kvs = function(branch, file, cb) {
  // Prepend branch name to the KV so that the subtree is properly namespaced in Consuls
  var key_name = create_key_name(branch, file);
  ...
  this.client.kv.get({'key': key_name + '/'}, function(err, kvs, res) {
 ...
 */
Consul.prototype.process_records = function(branch, records, cb) {
  var self = this;
    ...
    switch (record.type) {
      ...
      case 'T':
        ...
        if (record.path === branch.common_properties) {
          ...
          self.file_modified(branch, record.path, function(err) {
            branch.listAdditionalPropertyFiles(records, function(err, additionalRecords) {
              ...
              self.process_records(branch, additionalRecords, function(errs) {
          ...
          ++pending_records;
          self.file_modified(branch, record.path, check_pending);
        }
        break;
      case 'D':
        // Delete file
        ++pending_records;
        self.file_deleted(branch, record.path, check_pending);
        break;
 ...
 */
Consul.prototype.getLastProcessedRef = function(branch, cb) {
  var key_name = create_key_name(branch, branch.name + '.ref', true);
  this.client.kv.get({'key': key_name}, function(err, item) {
    /* istanbul ignore if */
    if (err) return cb(err);
 ...
 */
Consul.prototype.setLastProcessedRef = function(branch, ref, cb) {
  write_content_to_consul(this.client, create_key_name(branch, branch.name + '.ref', true), ref, cb);
};
 ...
 */
Consul.prototype.handleRefChange = function(branch, cb) {
  var self = this;
  ...
  branch.getCurrentRef(function(err, ref) {
    ...
    var handle_records = function(err, records) {
      /* istanbul ignore if */
      if (err) return cb(err);
      self.process_records(branch, records, function(errs) {
        if (errs) {
        ...
        self.setLastProcessedRef(branch, ref, function(err) {
          return cb(err);
        });
      });
    };
    ...
    self.getLastProcessedRef(branch, function(err, last_processed_ref) {
      /* istanbul ignore if */
      if (err) return cb(err);
